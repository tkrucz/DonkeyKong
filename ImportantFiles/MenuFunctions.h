...
void displayMainMenu(Stage* stage, SDLConst* SDL, Color* colors, Animator* animator, Score* score) {
	if (stage->menu.showMenu)
	{
		...
		DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO, MENU_TITLE_ROW,
		           stage->menu.text, SDL->charset);
		sprintf(stage->menu.text, "START NEW GAME - N");
		DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO, MENU_START_KEY_ROW,
		           stage->menu.text, SDL->charset);
		displayNamePart(stage, SDL);
		displayStagePart(stage, SDL);
		sprintf(stage->menu.text, "CHECK THE BEST SCORES - P");
		DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO, MENU_SCORE_KEY_ROW,
		           stage->menu.text, SDL->charset);
		sprintf(stage->menu.text, "LOAD STAGE FROM FILE - F");
		DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO,
		           MENU_STAGE_FROM_FILE_KEY_ROW, stage->menu.text, SDL->charset);
		sprintf(stage->menu.text, "EXIT THE GAME - ESC");
		DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO, MENU_EXIT_KEY_ROW,
		           stage->menu.text, SDL->charset);
		sprintf(stage->menu.text, "%s", stage->menu.message);
		DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO, MENU_MESSAGE_ROW,
		           stage->menu.text, SDL->charset);
...
void displayNamePart(Stage* stage, SDLConst* SDL) {
	if (!stage->menu.nameConfirmed) 
	{
		...
		DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO, MENU_NAME_ROW,
		           stage->menu.text, SDL->charset);
	} else 
	{
...
void displayStagePart(Stage* stage, SDLConst* SDL) {
	...
	DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO, MENU_STAGE_ROW,
	           stage->menu.text, SDL->charset);
	...
	if (stage->menu.stageChoose)
	{
		...
		DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO, MENU_CHOOSE_STAGE_ROW,
		           stage->menu.text, SDL->charset);
	}
}
		...
		switch (SDL->event.type) {
			case SDL_KEYDOWN:
				if (keyPressed == SDLK_ESCAPE)
					...
					if (stage->stageSpecifier == STAGE1 || stage->stageSpecifier == STAGE2 ||
					    stage->stageSpecifier == STAGE3 || stage->stageSpecifier == STAGE4)
...
void displayHittedByBarrelMenu(Stage* stage, SDLConst* SDL)
{
	if (stage->menu.showBarrelMenu) 
	{
		...
		DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO, BARREL_MENU_PLAYER_SCORE,
		           stage->menu.text, SDL->charset);
		sprintf(stage->menu.text, "You are hitted by barrel");
		DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO, BARREL_MENU_ALERT,
		           stage->menu.text, SDL->charset);
		sprintf(stage->menu.text, "You want to continue?");
		DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO, BARREL_MENU_CONTINUE,
		           stage->menu.text, SDL->charset);
		sprintf(stage->menu.text, "YES - Y , NO (quit the game) - Esc, NO (save score and then quit) - S");
		DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO, BARREL_MENU_KEYS,
		           stage->menu.text, SDL->charset);
...
void finishGameMenu(Stage* stage, SDLConst* SDL, Score* score)
{
	if (stage->stageSpecifier == STAGE3 && score->endTheStage == 0)
	{
		...
		DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO, FINISH_GAME_ROW,
		           stage->menu.text, SDL->charset);
		sprintf(stage->menu.text, "Player name (edit by pressing W, confirm by pressing ENTER) : %s",
		        stage->player.name);
		DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO, FINISH_NAME_ROW,
		           stage->menu.text, SDL->charset);
		sprintf(stage->menu.text, "Player score : %.6d", stage->playerInfo.score);
		DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO, FINISH_SCORE_ROW,
		           stage->menu.text, SDL->charset);
		sprintf(stage->menu.text, "Game Time %.1lf s", stage->gameInfo.gameTime);
		DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO, FINISH_TIME_ROW,
		           stage->menu.text, SDL->charset);
		sprintf(stage->menu.text, "ESC-quit, S- save the game result");
		DrawString(SDL->screen, SDL->screen->w / TWO - strlen(stage->menu.text) * EIGHT / TWO, FINISH_KEYS_ROW,
		           stage->menu.text, SDL->charset);
...
void loadStageFromFile(Stage* stage, SDLConst* SDL, Animator* animator, Color* colors) 
{
	...
	stage->platforms = new Platform[platformCounter];
	stage->ladders = new Ladder[laddersCounter];
	stage->barrels = new Barrel[barrelsCounter];
	stage->trophies = new Trophy[trophiesCounter];
	...
	for (int j = 0; j < stage->numberOfRowsInStageFile; j++) 
	{
		...
		{
			case 'P':
					stage->platforms[stage->numberOfPlatformsInFile++] = {x, y, w, z};
				break;
			case 'L':
				stage->ladders[stage->numberOfLaddersInFile++] = {x, y, LADDER_WIDTH, w};
				break;
			case 'B':
				stage->barrels[stage->numberOfBarrelsInFile++] = {x, y};
				break;
			case 'T':
				stage->trophies[stage->numberOfTrohpiesInFile++] = {x, y - TROPHIES_DIFFERENCE_IN_Y};
				break;
			case '*':
				break;
		}
